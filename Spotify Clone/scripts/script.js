...
async function getSongs(folder) {
  ...
  try {
    ...
    currFolder = folder;
    let songsData = await fetch(`http://192.168.0.118:52600/${folder}/`);
...
async function displayAlbums() {
  let songsData = await fetch(`http://192.168.0.118:52600/songs/`);
  let response = await songsData.text();
  let div = document.createElement("div");
  div.innerHTML = response;
  ...
  let array = Array.from(anchors)
    ...
    if (e.href.includes("/songs/") && !e.href.endsWith("/songs/")) {
      ...
      let songsData = await fetch(`http://192.168.0.118:52600/songs/${folder}/info.json`);
